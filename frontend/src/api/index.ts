...
const $v1 = axios.create({
  baseURL: `${import.meta.env.VITE_API_URL ?? 'http://localhost:7777'}/api/v1`,
...
$v1.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response?.status == 401 && error.config && !error.config._isRetry) {
      ...
      try {
        const response: AxiosResponse<UserLoginType> = await axios.get(`${import.meta.env.VITE_API_URL ?? 'http://localhost:7777'}/api/v1/refresh`, { withCredentials: true })
